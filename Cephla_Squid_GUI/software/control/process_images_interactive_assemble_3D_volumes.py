...
basePath = Path(input('Enter base path: '))
# # in raw folder
...
def stackVolumePlanes(folderPath, basePath, started, ptName, channelFilters, startTime, 
...
if useCoordNames and tileY > 1 and tileX > 1: # multipoint tiles
    ...
    with tqdm(total = len(coords)) as pbar:
        for ii, row1 in coords.iterrows(): # each coord
            ...
            startTime = time.time()
            stackVolumePlanes(folderPath, basePath, started, ptName, channelFilters, startTime, 
                                yTileDim, xTileDim, tileY, tileX, overlapFrac)
