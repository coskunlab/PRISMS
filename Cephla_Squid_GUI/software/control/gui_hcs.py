...
class HighContentScreeningGui(QMainWindow):
    ...
    def __init__(self, is_simulation=False, live_only_mode=False, *args, **kwargs):
        ...
        # Initialize channel selection based on current exposure times
        if USE_NAPARI_FOR_LIVE_VIEW:
            self.napariLiveWidget.emit_selected_channels()
    ...
    def loadHardwareObjects(self):
        ...
        #         sys.exit(1)
        self.pump = serial_peripherals.HarvardPump()
        self.pump.connect()
        print("\n--- Configuring Pump ---")
        # Use methods with lowercase commands as per manual examples
        self.pump.set_diameter(SYRINGE_DIAMETER_MM)
        time.sleep(0.2)
        self.pump.set_infusion_rate(INFUSION_RATE_VALUE, INFUSION_RATE_UNIT)
        time.sleep(0.2)
        # No direction command needed
        print("\n--- Starting Infusion (will run indefinitely) ---")
        self.pump.start_infuse() # Use the specific infusion start command
        print("Pump infuse command sent.")
        print("Infusing continuously")
    ...
    def setupRecordTabWidget(self):
        ...
        # Remove flexibleMultiPointWidget from here since it's now in navigationTabWidget
    ...
    def setupLayout(self):
        ...
        # Comment out sampleSettingsWidget to give FlexibleMultiPointWidget more room
        # layout.addWidget(self.sampleSettingsWidget)
        
        # Create a new tab widget for navigationViewer and flexibleMultiPointWidget
        self.navigationTabWidget = QTabWidget()
        self.navigationTabWidget.addTab(self.flexibleMultiPointWidget, "Flexible Multipoint")
        self.navigationTabWidget.addTab(self.navigationViewer, "Navigation")
        layout.addWidget(self.navigationTabWidget)
    ...
    def makeConnections(self):
        ...
        if ENABLE_FLEXIBLE_MULTIPOINT:
            ...
            # Connect the configurations updated signal to refresh the Live Configuration widget
            self.flexibleMultiPointWidget.signal_configurations_updated.connect(
                self.liveControlWidget.refresh_current_configuration_display
            )
            # Also emit selected channels when configurations are updated to reflect exposure time changes
            self.flexibleMultiPointWidget.signal_configurations_updated.connect(
                self.liveControlWidget.emit_selected_channels
            )
            # Connect the sample channels changed signal to update the configuration display for the sample
            self.flexibleMultiPointWidget.signal_sample_channels_changed.connect(
                self.liveControlWidget.update_configuration_for_sample_channels
            )
            if ENABLE_STITCHER:
        ...
        # Connect configuration changes to channel selection updates (only if using Napari)
        if USE_NAPARI_FOR_LIVE_VIEW:
            self.liveControlWidget.signal_configuration_changed.connect(self.napariLiveWidget.on_configuration_changed)
        # Connect selected channels from LiveControlWidget to FlexibleMultiPointWidget
        self.liveControlWidget.signal_selected_channels.connect(self.flexibleMultiPointWidget.update_selected_channels)
        # Emit initial selected channels
        self.liveControlWidget.emit_selected_channels()
        if not self.live_only_mode:
            self.liveControlWidget.signal_start_live.connect(self.onStartLive)
        self.liveControlWidget.update_camera_settings()

        # Reset all channel exposure times to 0 on launch (deselected state) and refresh UI
        print("DEBUG: Resetting all channel exposure times to 0 on application launch")
        self.configurationManager.reset_all_exposure_times_to_zero()
        self.liveControlWidget.refresh_current_configuration_display()
        self.liveControlWidget.emit_selected_channels()  # Update selected channels based on reset exposure times
        ...
        # Connect navigation tab widget to trigger updates when switching between navigation and flexible multipoint
        if hasattr(self, 'navigationTabWidget'):
            self.navigationTabWidget.currentChanged.connect(lambda: self.onTabChanged(self.recordTabWidget.currentIndex()))
        if not self.live_only_mode:
    ...
    def onTabChanged(self, index):
        # Check if flexible multipoint is active in the navigation tab widget
        is_flexible_acquisition = (
            self.navigationTabWidget.currentWidget() == self.flexibleMultiPointWidget
            if hasattr(self, 'navigationTabWidget') and ENABLE_FLEXIBLE_MULTIPOINT
            else False
        )
        ...
        # Get acquisition widget - check both record tab and navigation tab
        acquisitionWidget = None
        if is_flexible_acquisition:
            acquisitionWidget = self.flexibleMultiPointWidget
        else:
            acquisitionWidget = self.recordTabWidget.widget(index)
            
        if acquisitionWidget and ENABLE_STITCHER:
            self.toggleStitcherWidget(acquisitionWidget.checkbox_stitchOutput.isChecked())
        if acquisitionWidget and hasattr(acquisitionWidget, 'emit_selected_channels'):
