...
class LiveControlWidget(QFrame):
    ...
    signal_configuration_changed = Signal(str, str, object)  # config_name, parameter, value
    signal_selected_channels = Signal(list)  # channels with exposure times > 0
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.fps_trigger = 1 # reduces software crashing
        self.fps_display = 10
    ...
    def update_config_exposure_time(self, new_value):
        if self.is_switching_mode == False:
            ...
            # Emit signal when any configuration's exposure time changes
            self.signal_configuration_changed.emit(self.currentConfiguration.name, 'exposure_time', new_value)
            # Emit selected channels based on updated exposure times
    ...
    def get_selected_channels(self):
        """Return list of configurations where exposure time > 0"""
        selected_channels = []
        for config in self.configurationManager.configurations:
            if config.exposure_time > 0:
                # Create channel object with name and exposure time
                channel_info = {
                    'name': config.name,
                    ...
                    'z_offset': config.z_offset if hasattr(config, 'z_offset') else 0
                }
                selected_channels.append(channel_info)
        return selected_channels

    def emit_selected_channels(self):
        """Emit signal with currently selected channels based on exposure times"""
        selected_channels = self.get_selected_channels()
        self.signal_selected_channels.emit(selected_channels)

    def refresh_current_configuration_display(self):
        """Refresh the UI to display the current configuration values"""
        if self.currentConfiguration:
            print(f"DEBUG: Refreshing configuration display for '{self.currentConfiguration.name}' - Exposure: {self.currentConfiguration.exposure_time}ms, Gain: {self.currentConfiguration.analog_gain}, Illumination: {self.currentConfiguration.illumination_intensity}%")
            self.is_switching_mode = True
            # Update the UI elements with current configuration values
            self.entry_exposureTime.setValue(self.currentConfiguration.exposure_time)
            self.entry_analogGain.setValue(self.currentConfiguration.analog_gain)
            self.entry_illuminationIntensity.setValue(self.currentConfiguration.illumination_intensity)
            self.is_switching_mode = False
        else:
            print("DEBUG: No current configuration to display")

    def update_configuration_for_sample_channels(self, sample_channels):
        """Update the Live Configuration dropdown to show a relevant configuration for the current sample"""
        if not sample_channels:
            print("DEBUG: No sample channels provided to update_configuration_for_sample_channels")
            return
            
        print(f"DEBUG: Updating configuration for sample with {len(sample_channels)} channels:")
        
        # Find the first active channel (with exposure_time > 0) from this sample
        active_channel_name = None
        for channel in sample_channels:
            if isinstance(channel, dict):
                channel_name = channel.get('name')
                exposure_time = channel.get('exposure_time', 0)
                print(f"DEBUG: Checking channel '{channel_name}' - Exposure: {exposure_time}ms")
                if exposure_time > 0:
                    active_channel_name = channel_name
                    print(f"DEBUG: Found active channel: '{active_channel_name}'")
                    break
        
        # If we found an active channel, switch the dropdown to show that configuration
        if active_channel_name:
            print(f"DEBUG: Switching Live Configuration dropdown to '{active_channel_name}'")
            # Find the configuration that matches this channel name
            for config in self.configurationManager.configurations:
                if config.name == active_channel_name:
                    # Update the dropdown selection (this will trigger update_microscope_mode_by_name)
                    self.dropdown_modeSelection.setCurrentText(config.name)
                    break
        else:
            print("DEBUG: No active channels found (all exposure times are 0)")
...
class NavigationWidget(QFrame):
    ...
    def add_components(self):
        ...
        grid_line4 = QHBoxLayout()
        self.btn_purge_water = QPushButton("Purge Water")
        grid_line4.addWidget(self.btn_purge_water)
        self.grid.addLayout(grid_line4)
        self.btn_purge_water.clicked.connect(self.purgePump)

    # purge water for lens
    def startPumpConst(self, speed = 10): # uL/min
        if self.pump is not None:
            self.pump.set_infusion_rate(speed, 'UM')
            time.sleep(0.2)
            self.pump.start_infuse() # continuous infusion

    # purge pump: high vol, short time
    def purgePump(self): # uL/s, sec
        speed = 100
        duration = 1
        if self.pump is not None:
            print('Purging pump at', speed, 'uL/s', 'for', duration, 'sec')
            self.pump.set_infusion_rate(speed, 'ul/s')
            time.sleep(0.2)
            self.pump.start_infuse() # continuous infusion
            time.sleep(duration)
            # reset to base speed
...
class FlexibleMultiPointWidget(QFrame):
    ...
    signal_configurations_updated = Signal()  # Signal to notify when configurations are updated
    signal_sample_channels_changed = Signal(list)  # Signal to notify when sample channels change
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.sample_list = []  # List of sample dictionaries
        self.current_sample_index = 0
        self.selected_channels = []  # Current selected channels from exposure times
        ...
        self.selected_channels = []  # Store selected channels from LiveControlWidget based on exposure times
    ...
    def add_components(self):
        ...
        self.btn_offset_current_XY = QPushButton('Offset current XY')
        self.btn_offset_current_Z = QPushButton('Offset current Z')
        self.btn_offset_all_XY = QPushButton('Offset ALL XY')
        self.btn_offset_all_Z = QPushButton('Offset ALL Z')
        ...
        # NEW COMPONENTS FOR setExpParams FUNCTION
        # Integer inputs
        self.entry_focusCount = QSpinBox()
        self.entry_focusCount.setMinimum(1)
        self.entry_focusCount.setMaximum(1000)
        self.entry_focusCount.setSingleStep(1)
        self.entry_focusCount.setValue(100)
        self.entry_focusCount.setKeyboardTracking(False)
        
        self.entry_zRetract = QSpinBox()
        self.entry_zRetract.setMinimum(-10000)
        self.entry_zRetract.setMaximum(0)
        self.entry_zRetract.setSingleStep(100)
        self.entry_zRetract.setValue(-5000)
        self.entry_zRetract.setKeyboardTracking(False)
        
        self.entry_objective = QSpinBox()
        self.entry_objective.setMinimum(1)
        self.entry_objective.setMaximum(200)
        self.entry_objective.setSingleStep(1)
        self.entry_objective.setValue(60)
        self.entry_objective.setKeyboardTracking(False)
        
        self.entry_initialOffset = QSpinBox()
        self.entry_initialOffset.setMinimum(-1000)
        self.entry_initialOffset.setMaximum(1000)
        self.entry_initialOffset.setSingleStep(1)
        self.entry_initialOffset.setValue(0)
        self.entry_initialOffset.setKeyboardTracking(False)
        
        self.entry_numFrames = QSpinBox()
        self.entry_numFrames.setMinimum(1)
        self.entry_numFrames.setMaximum(100)
        self.entry_numFrames.setSingleStep(1)
        self.entry_numFrames.setValue(1)
        self.entry_numFrames.setKeyboardTracking(False)
        
        # Float input
        self.entry_overlapFrac = QDoubleSpinBox()
        self.entry_overlapFrac.setMinimum(0.0)
        self.entry_overlapFrac.setMaximum(0.9)
        self.entry_overlapFrac.setSingleStep(0.01)
        self.entry_overlapFrac.setValue(0.15)
        self.entry_overlapFrac.setDecimals(3)
        self.entry_overlapFrac.setKeyboardTracking(False)
        
        # Dropdown menus
        self.dropdown_focusChan = QComboBox()
        # Use the same configuration options as Live Configuration
        if self.configurationManager:
            for configuration in self.configurationManager.configurations:
                self.dropdown_focusChan.addItem(configuration.name)
            # Set default to first configuration if available
            if self.configurationManager.configurations:
                self.dropdown_focusChan.setCurrentText(self.configurationManager.configurations[0].name)
        else:
            # Fallback to hardcoded options if configurationManager is not available
            self.dropdown_focusChan.addItems([
            ...
            self.dropdown_focusChan.setCurrentText('Fluorescence 405 nm Ex')
        
        self.dropdown_stitchMode = QComboBox()
        self.dropdown_stitchMode.addItems(['MIP', 'BestFocus'])
        self.dropdown_stitchMode.setCurrentText('MIP')
        
        # Checkboxes
        self.checkbox_fillBoundary = QCheckBox('Fill Boundary')
        self.checkbox_fillBoundary.setChecked(False)
        
        self.checkbox_spreadOil = QCheckBox('Spread Oil')
        self.checkbox_spreadOil.setChecked(False)
        
        self.checkbox_registerToPrevTile = QCheckBox('Register to Previous Tile')
        self.checkbox_registerToPrevTile.setChecked(False)
        
        # SAMPLE MANAGEMENT UI COMPONENTS
        self.sample_list_widget = QListWidget()
        self.sample_list_widget.setMaximumHeight(100)
        
        self.btn_add_sample = QPushButton('Add Sample')
        self.btn_remove_sample = QPushButton('Remove Sample')
        self.btn_edit_sample = QPushButton('Update Sample')
        self.btn_duplicate_sample = QPushButton('Duplicate Sample')
        self.btn_validate_samples = QPushButton('Validate Samples')
        
        self.label_current_sample = QLabel('Sample 1 of 1')
        self.label_current_sample.setAlignment(Qt.AlignCenter)
        ...
        grid_line3point5.addWidget(self.btn_previous,0,2)
        grid_line3point5.addWidget(self.btn_goTo,0,3)
        grid_line3point5.addWidget(self.btn_next,0,4)
        #grid_line3point5.addWidget(self.btn_load_last_executed,0,4)

        # buttons to offset current and all coords
        grid_line_offset_coords = QGridLayout()
        grid_line_offset_coords.addWidget(self.btn_offset_current_XY,0,0)
        grid_line_offset_coords.addWidget(self.btn_offset_current_Z,0,1)
        grid_line_offset_coords.addWidget(self.btn_offset_all_XY,0,2)
        grid_line_offset_coords.addWidget(self.btn_offset_all_Z,0,3)
        ...
        grid_line2 = QGridLayout()
        grid_line2.addWidget(QLabel('Nx'), 0,0)
        grid_line2.addWidget(self.entry_NX, 0,1)
        grid_line2.addWidget(QLabel('Ny'), 0,2)
        grid_line2.addWidget(self.entry_NY, 0,3)
        ...
        # NEW GRID FOR ADDITIONAL PARAMETERS
        grid_line_params1 = QGridLayout()
        grid_line_params1.addWidget(QLabel('Focus Count'), 0,0)
        grid_line_params1.addWidget(self.entry_focusCount, 0,1)
        grid_line_params1.addWidget(QLabel('Z Retract'), 0,2)
        grid_line_params1.addWidget(self.entry_zRetract, 0,3)
        
        grid_line_params1.addWidget(QLabel('Objective'), 1,0)
        grid_line_params1.addWidget(self.entry_objective, 1,1)
        grid_line_params1.addWidget(QLabel('Initial Offset'), 1,2)
        grid_line_params1.addWidget(self.entry_initialOffset, 1,3)
        
        grid_line_params2 = QGridLayout()
        grid_line_params2.addWidget(QLabel('Num Frames'), 0,0)
        grid_line_params2.addWidget(self.entry_numFrames, 0,1)
        grid_line_params2.addWidget(QLabel('Overlap Fraction'), 0,2)
        grid_line_params2.addWidget(self.entry_overlapFrac, 0,3)
        
        grid_line_params3 = QGridLayout()
        grid_line_params3.addWidget(QLabel('Focus Channel'), 0,0)
        grid_line_params3.addWidget(self.dropdown_focusChan, 0,1)
        grid_line_params3.addWidget(QLabel('Stitch Mode'), 0,2)
        grid_line_params3.addWidget(self.dropdown_stitchMode, 0,3)
        
        grid_line_checkboxes = QGridLayout()
        grid_line_checkboxes.addWidget(self.checkbox_fillBoundary, 0,0)
        grid_line_checkboxes.addWidget(self.checkbox_spreadOil, 0,1)
        grid_line_checkboxes.addWidget(self.checkbox_registerToPrevTile, 0,2)

        # SAMPLE MANAGEMENT LAYOUTS
        grid_line_sample_controls = QGridLayout()
        grid_line_sample_controls.addWidget(self.btn_add_sample, 0,0)
        grid_line_sample_controls.addWidget(self.btn_remove_sample, 0,1)
        grid_line_sample_controls.addWidget(self.btn_edit_sample, 0,2)
        grid_line_sample_controls.addWidget(self.btn_duplicate_sample, 0,3)
        grid_line_sample_controls.addWidget(self.btn_validate_samples, 1,0)
        
        grid_line_sample_list = QVBoxLayout()
        grid_line_sample_list.addWidget(QLabel('Sample Sequence:'))
        grid_line_sample_list.addWidget(self.sample_list_widget)
        grid_line_sample_list.addWidget(self.label_current_sample)
        ...
        self.grid.addLayout(grid_line_offset_coords,3,0)
        self.grid.addLayout(grid_line3point75,4,0)
        # self.grid.addLayout(grid_line5,2,0)
        self.grid.addLayout(grid_line2,5,0)
        self.grid.addLayout(grid_line_params1,6,0)
        self.grid.addLayout(grid_line_params2,7,0)
        self.grid.addLayout(grid_line_params3,8,0)
        self.grid.addLayout(grid_line_checkboxes,9,0)
        self.grid.addLayout(grid_line_sample_controls,10,0)
        self.grid.addLayout(grid_line_sample_list,11,0)
        self.grid.addLayout(grid_line3,12,0)
        
        # Create scrollable widget
        scroll_widget = QWidget()
        scroll_widget.setLayout(self.grid)
        
        # Create scroll area (store as class member for hover functionality)
        self.scroll_area = QScrollArea()
        ...
        # self.set_minZ_button.clicked.connect(self.set_z_min) # disable
        ...
        self.entry_focusCount.valueChanged.connect(self.set_focusCount)
        self.entry_zRetract.valueChanged.connect(self.set_zRetract)
        self.entry_objective.valueChanged.connect(self.set_objective)
        self.entry_initialOffset.valueChanged.connect(self.set_initialOffset)
        self.entry_numFrames.valueChanged.connect(self.set_numFrames)
        self.entry_overlapFrac.valueChanged.connect(self.set_overlapFrac)
        self.dropdown_focusChan.currentTextChanged.connect(self.set_focusChan)
        self.dropdown_stitchMode.currentTextChanged.connect(self.set_stitchMode)
        self.checkbox_fillBoundary.toggled.connect(self.set_fillBoundary)
        self.checkbox_spreadOil.toggled.connect(self.set_spreadOil)
        self.checkbox_registerToPrevTile.toggled.connect(self.set_registerToPrevTile)
        
        # SAMPLE MANAGEMENT SIGNAL CONNECTIONS
        self.btn_add_sample.clicked.connect(self.add_sample)
        self.btn_remove_sample.clicked.connect(self.remove_sample)
        self.btn_edit_sample.clicked.connect(self.edit_sample)
        self.btn_duplicate_sample.clicked.connect(self.duplicate_sample)
        self.btn_validate_samples.clicked.connect(self.validate_samples_dialog)
        self.sample_list_widget.itemClicked.connect(self.select_sample)
        ...
        self.btn_goTo.clicked.connect(self.go_to)
        ...
        self.btn_offset_current_XY.clicked.connect(self.offset_current_XY) # coord offset buttons
        self.btn_offset_current_Z.clicked.connect(self.offset_current_Z)
        self.btn_offset_all_XY.clicked.connect(self.offset_all_XY)
        self.btn_offset_all_Z.clicked.connect(self.offset_all_Z)
    ...
    def update_selected_channels(self, selected_channels):
        """Receive selected channels from LiveControlWidget based on exposure times"""
        self.selected_channels = selected_channels
        # Emit the signal to notify other components about channel selection
        self.signal_acquisition_channels.emit(selected_channels)
        print(f"FlexibleMultiPointWidget: Updated selected channels: {selected_channels}")
    ...
    def set_deltaZ(self, value):
        # multipointController.set_deltaZ expects value in micrometers (μm)
        self.multipointController.set_deltaZ(value)

    # NEW METHODS FOR setExpParams PARAMETERS
    def set_focusCount(self, value):
        self.multipointController.set_focusCount(value)
    
    def set_zRetract(self, value):
        self.multipointController.set_zRetract(value)
    
    def set_objective(self, value):
        self.multipointController.set_objective(value)
    
    def set_initialOffset(self, value):
        self.multipointController.set_initialOffset(value)
    
    def set_numFrames(self, value):
        self.multipointController.set_numFrames(value)
    
    def set_overlapFrac(self, value):
        self.multipointController.set_overlapFrac(value)
    
    def set_focusChan(self, value):
        self.multipointController.set_focusChan(value)
    
    def set_stitchMode(self, value):
        self.multipointController.set_stitchMode(value)
    
    def set_fillBoundary(self, value):
        self.multipointController.set_fillBoundary(value)
        # Disable Nx and Ny when fillBoundary is True, enable when False
        self.entry_NX.setEnabled(not value)
        self.entry_NY.setEnabled(not value)
    
    def set_spreadOil(self, value):
        self.multipointController.set_spreadOil(value)
    
    def set_registerToPrevTile(self, value):
        self.multipointController.set_registerToPrevTile(value)

    # SAMPLE MANAGEMENT METHODS
    def get_current_sample_config(self):
        """Get current sample configuration from UI"""
        # Create selected_channels dictionary with exposure times for validation
        selected_channels_dict = {}
        if hasattr(self, 'selected_channels') and self.selected_channels:
            for channel in self.selected_channels:
                if isinstance(channel, dict):
                    selected_channels_dict[channel.get('name', 'Unknown')] = channel.get('exposure_time', 0)
                elif hasattr(channel, 'name') and hasattr(channel, 'exposure_time'):
                    selected_channels_dict[channel.name] = channel.exposure_time
        
        return {
            'name': f"Sample {len(self.sample_list) + 1}",
            'channels': self.selected_channels.copy() if hasattr(self, 'selected_channels') else [],
            'selected_channels': selected_channels_dict,  # For validation
            ...
            'focusCount': self.entry_focusCount.value(),
            'zRetract': self.entry_zRetract.value(),
            'objective': self.entry_objective.value(),
            'initialOffset': self.entry_initialOffset.value(),
            'numFrames': self.entry_numFrames.value(),
            'overlapFrac': self.entry_overlapFrac.value(),
            'focusChan': self.dropdown_focusChan.currentText(),
            'stitchMode': self.dropdown_stitchMode.currentText(),
            'fillBoundary': self.checkbox_fillBoundary.isChecked(),
            'spreadOil': self.checkbox_spreadOil.isChecked(),
            'registerToPrevTile': self.checkbox_registerToPrevTile.isChecked(),
            'location_list': self.location_list.copy(),
            'location_ids': self.location_ids.copy()
        }
    
    def apply_sample_config(self, sample_config):
        """Apply sample configuration to UI"""
        self.selected_channels = sample_config['channels'].copy()
        
        # Update configuration manager with saved channel exposure times
        self.apply_sample_channels_to_configuration()
        
        # Set base path if available
        if 'base_path' in sample_config and sample_config['base_path']:
            self.lineEdit_savingDir.setText(sample_config['base_path'])
            self.multipointController.set_base_path(sample_config['base_path'])
            self.base_path_is_set = True
        self.entry_NX.setValue(sample_config['nx'])
        self.entry_NY.setValue(sample_config['ny'])
        self.entry_NZ.setValue(sample_config['nz'])
        self.entry_deltaZ.setValue(sample_config['dz'])
        self.entry_focusCount.setValue(sample_config['focusCount'])
        self.entry_zRetract.setValue(sample_config['zRetract'])
        self.entry_objective.setValue(sample_config['objective'])
        self.entry_initialOffset.setValue(sample_config['initialOffset'])
        self.entry_numFrames.setValue(sample_config['numFrames'])
        self.entry_overlapFrac.setValue(sample_config['overlapFrac'])
        self.dropdown_focusChan.setCurrentText(sample_config['focusChan'])
        self.dropdown_stitchMode.setCurrentText(sample_config['stitchMode'])
        self.checkbox_fillBoundary.setChecked(sample_config['fillBoundary'])
        self.checkbox_spreadOil.setChecked(sample_config['spreadOil'])
        self.checkbox_registerToPrevTile.setChecked(sample_config['registerToPrevTile'])
        self.location_list = sample_config['location_list'].copy()
        self.location_ids = sample_config['location_ids'].copy()
        self.update_GUI_location_list()
        
        # Force an additional signal emission to ensure LiveControlWidget refreshes
        print("DEBUG: Emitting additional signal_configurations_updated after applying sample config")
        self.signal_configurations_updated.emit()
        
        # Also emit the selected channels signal to ensure UI reflects the updated exposure times
        print("DEBUG: Emitting signal_sample_channels_changed with updated channels")
        self.signal_sample_channels_changed.emit(self.selected_channels)

    def apply_sample_channels_to_configuration(self):
        """Apply sample's channel exposure times to configuration manager"""
        if not hasattr(self, 'selected_channels') or not self.selected_channels:
            print("DEBUG: No selected channels to apply to configuration")
            return
            
        print(f"DEBUG: Applying sample channels to configuration (total channels: {len(self.selected_channels)})")
        
        # Update configuration manager with exposure times from saved channels
        for channel in self.selected_channels:
            if isinstance(channel, dict):
                channel_name = channel.get('name')
                exposure_time = channel.get('exposure_time', 0)
                analog_gain = channel.get('analog_gain', 0)
                illumination_intensity = channel.get('illumination_intensity', 100)
                
                print(f"DEBUG: Channel '{channel_name}' - Exposure: {exposure_time}ms, Gain: {analog_gain}, Illumination: {illumination_intensity}%")
                
                # Find matching configuration
                config_found = False
                for config in self.configurationManager.configurations:
                    if config.name == channel_name:
                        # Debug: Show config values before update
                        print(f"DEBUG: Before update - '{channel_name}' exposure: {config.exposure_time}, target: {exposure_time}")
                        
                        # Update configuration with saved values
                        self.configurationManager.update_configuration(config.id, "ExposureTime", exposure_time)
                        self.configurationManager.update_configuration(config.id, "AnalogGain", analog_gain)
                        self.configurationManager.update_configuration(config.id, "IlluminationIntensity", illumination_intensity)
                        
                        # Debug: Show config values after update
                        print(f"DEBUG: After update - '{channel_name}' exposure: {config.exposure_time}, analog_gain: {config.analog_gain}, illumination: {config.illumination_intensity}")
                        config_found = True
                        break
                
                if not config_found:
                    print(f"DEBUG: WARNING - No configuration found for channel '{channel_name}'")
        
        # Emit signals to notify that configurations have been updated
        print("DEBUG: Emitting signal_configurations_updated from apply_sample_channels_to_configuration")
        self.signal_configurations_updated.emit()
        self.signal_sample_channels_changed.emit(self.selected_channels)
    
    def add_sample(self):
        """Add current configuration as a new sample"""
        sample_config = self.get_current_sample_config()
        self.sample_list.append(sample_config)
        self.update_sample_list_display()
        
        # Reset all channel exposure times to 0 after adding sample (deselected state)
        print("DEBUG: Resetting all channel exposure times to 0 after adding sample")
        if hasattr(self, 'configurationManager') and self.configurationManager:
            self.configurationManager.reset_all_exposure_times_to_zero()
            # Emit signal to notify that configurations have been updated (this will refresh the LiveControlWidget)
            self.signal_configurations_updated.emit()
    
    def remove_sample(self):
        """Remove selected sample"""
        if self.sample_list_widget.currentRow() >= 0 and self.sample_list:
            index = self.sample_list_widget.currentRow()
            del self.sample_list[index]
            if self.current_sample_index >= len(self.sample_list) and self.sample_list:
                self.current_sample_index = len(self.sample_list) - 1
            elif not self.sample_list:
                self.current_sample_index = 0
            self.update_sample_list_display()
            if self.sample_list:
                self.apply_sample_config(self.sample_list[self.current_sample_index])
    
    def edit_sample(self):
        """Update selected sample with current configuration"""
        if self.sample_list_widget.currentRow() >= 0 and self.sample_list:
            index = self.sample_list_widget.currentRow()
            sample_config = self.get_current_sample_config()
            sample_config['name'] = self.sample_list[index]['name']  # Keep original name
            self.sample_list[index] = sample_config
            self.update_sample_list_display()
    ...
    def duplicate_sample(self):
        ...
        if self.sample_list_widget.currentRow() >= 0 and self.sample_list:
            index = self.sample_list_widget.currentRow()
            sample_config = self.sample_list[index].copy()
            sample_config['name'] = f"Sample {len(self.sample_list) + 1}"
            self.sample_list.append(sample_config)
            self.update_sample_list_display()
    
    def select_sample(self, item):
        """Switch to selected sample"""
        index = self.sample_list_widget.row(item)
        if 0 <= index < len(self.sample_list):
            sample = self.sample_list[index]
            print(f"DEBUG: ===== Selecting sample {index + 1} =====")
            print(f"DEBUG: Sample name: {sample.get('name', 'Unnamed')}")
            if sample.get('channels'):
                print(f"DEBUG: Sample has {len(sample['channels'])} channels:")
                for ch in sample['channels']:
                    if isinstance(ch, dict):
                        print(f"DEBUG:   - {ch.get('name', 'Unknown')}: {ch.get('exposure_time', 0)}ms")
            else:
                print("DEBUG: Sample has no channels configured")
            
            self.current_sample_index = index
            self.apply_sample_config(sample)
            self.update_sample_status_label()
    
    def update_sample_list_display(self):
        """Update the sample list widget"""
        self.sample_list_widget.clear()
        for i, sample in enumerate(self.sample_list):
            # Handle channels as dictionaries with 'name' key
            if sample['channels'] and isinstance(sample['channels'][0], dict):
                channels_str = ', '.join([ch['name'] for ch in sample['channels']])
            else:
                # Fallback for if channels are strings or objects with .name attribute
                channels_str = ', '.join([ch.name if hasattr(ch, 'name') else str(ch) for ch in sample['channels']])
            item_text = f"{sample['name']} ({channels_str})"
            self.sample_list_widget.addItem(item_text)
        self.update_sample_status_label()
    
    def update_sample_status_label(self):
        """Update the current sample status label"""
        if self.sample_list:
            self.label_current_sample.setText(f"Sample {self.current_sample_index + 1} of {len(self.sample_list)}")
        else:
    ...
    def validate_sample(self, sample_config):
        ...
        # Check base path
        base_path = sample_config.get('base_path', '')
        if not base_path or base_path == 'Choose a base saving directory':
            errors.append("Base path not set")
        elif not os.path.exists(base_path):
            errors.append(f"Base path does not exist: {base_path}")
        
        # Check selected channels with exposure times > 0
        selected_channels = sample_config.get('selected_channels', {})
        valid_channels = []
        if selected_channels:
            for channel_name, exposure_time in selected_channels.items():
                if exposure_time > 0:
    ...
    def validate_all_samples(self):
        """Validate all samples in the sample list"""
        if not self.sample_list:
            return False, ["No samples defined"]
        
        error_messages = []
        for i, sample in enumerate(self.sample_list):
            is_valid, error_msg = self.validate_sample(sample)
            if not is_valid:
                error_messages.append(f"Sample {i+1}: {error_msg}")
    ...
    def validate_samples_dialog(self):
        ...
        is_valid, error_messages = self.validate_all_samples()
        
        msg = QMessageBox()
        msg.setWindowTitle("Sample Validation")
        
        if is_valid:
            msg.setText("All samples are valid!")
            msg.setIcon(QMessageBox.Information)
        else:
            msg.setText("Sample validation errors found:")
            msg.setDetailedText("\n".join(error_messages))
    ...
    def emit_selected_channels(self):
        """Emit signal with currently selected channels"""
        self.signal_acquisition_channels.emit(self.selected_channels)
    ...
    def toggle_acquisition(self,pressed):
        if self.base_path_is_set == False:
            ...
            return
        if pressed:
            # Validate samples before starting acquisition
            if self.sample_list:
                is_valid, error_messages = self.validate_all_samples()
                if not is_valid:
                    self.btn_startAcquisition.setChecked(False)
                    msg = QMessageBox()
                    msg.setWindowTitle("Sample Validation Error")
                    msg.setText("Cannot start acquisition due to invalid sample configurations:")
                    msg.setDetailedText("\n".join(error_messages))
                    msg.setIcon(QMessageBox.Warning)
                    msg.exec_()
                    return
            else:
                # Validate current configuration if no samples defined
                current_sample = self.get_current_sample_config()
                is_valid, error_message = self.validate_sample(current_sample)
                if not is_valid:
                    self.btn_startAcquisition.setChecked(False)
                    msg = QMessageBox()
                    msg.setWindowTitle("Configuration Validation Error")
                    msg.setText("Cannot start acquisition due to invalid configuration:")
            ...
            # Prepare sample list for acquisition
            if self.sample_list:
                # Use defined samples
                sample_list_for_acquisition = self.sample_list.copy()
            else:
                # Create single sample from current configuration
                current_sample = self.get_current_sample_config()
                current_sample['location_list'] = self.location_list.copy()
                current_sample['location_ids'] = self.location_ids.copy()
                sample_list_for_acquisition = [current_sample]
            
            # add the current location to the location list if the list is empty for current sample
            if len(self.location_list) == 0:
                self.add_location()
                self.acquisition_in_place = True
            
            # Pass sample list to multipoint controller
            self.multipointController.set_sample_list(sample_list_for_acquisition)
            self.multipointController.start_new_experiment('')  # Empty experiment ID since box removed
            self.signal_acquisition_started.emit(True)
            
            # Set parameters for first sample to emit shape signal
            first_sample = sample_list_for_acquisition[0]
            self.signal_acquisition_shape.emit(first_sample['nx'],
                                               first_sample['ny'],
                                               first_sample['nz'],
                                               0,  # deltaX not used
                                               0,  # deltaY not used
            ...
            self.multipointController.run_acquisition(sample_list_for_acquisition)
    ...
    def load_last_used_locations(self):
        ...
        for row, row_ind in zip(self.last_used_locations, self.last_used_location_ids):
            ...
            if not np.any(np.all(self.location_list[:, :2] == [x, y], axis=1)):
                location_str = 'x: ' + str(round(x,3)) + \
                    ' mm, y: ' + str(round(y,3)) + \
                        ' mm, z: ' + str(round(1000*z,1)) + ' um' + \
                            ', ID: ' + name
    ...
    def setEnabled_all(self,enabled,exclude_btn_startAcquisition=True):
        ...
        # NEW WIDGETS
        self.entry_focusCount.setEnabled(enabled)
        self.entry_zRetract.setEnabled(enabled)
        self.entry_objective.setEnabled(enabled)
        self.entry_initialOffset.setEnabled(enabled)
        self.entry_numFrames.setEnabled(enabled)
        self.entry_overlapFrac.setEnabled(enabled)
        self.dropdown_focusChan.setEnabled(enabled)
        self.dropdown_stitchMode.setEnabled(enabled)
        self.checkbox_fillBoundary.setEnabled(enabled)
        self.checkbox_spreadOil.setEnabled(enabled)
        self.checkbox_registerToPrevTile.setEnabled(enabled)
    ...
    def add_location(self):
        x = self.stage.get_pos().x_mm
        y = self.stage.get_pos().y_mm
        z = self.stage.get_pos().z_mm
        name = f"FOV_{len(self.location_list)}" # auto-generated multipoint name
        if self.scanCoordinates is not None:
            # name = self.create_point_id()
            name = f"FOV_{len(self.location_list)}"
        if not np.any(np.all(self.location_list[:, :2] == [x, y], axis=1)):
            location_str = 'x: ' + str(round(x,3)) + \
                ' mm, y: ' + str(round(y,3)) + \
                    ' mm, z: ' + str(round(1000*z,1)) + ' um' + \
            ...
            index = self.dropdown_location_list.count() - 1
            self.dropdown_location_list.setCurrentIndex(index)
            self.location_list = np.vstack((self.location_list, [[self.stage.get_pos().x_mm,self.stage.get_pos().y_mm,self.stage.get_pos().z_mm]]))
            print(self.location_list)
            self.location_ids = np.append(self.location_ids, name)
            self.table_location_list.insertRow(self.table_location_list.rowCount())
            self.table_location_list.setItem(self.table_location_list.rowCount()-1,0, QTableWidgetItem(str(round(x,3))))
    ...
    def write_fov_name(self):
        ...
        if self.location_list.shape[0] == 0:
            return
        index = self.dropdown_location_list.currentIndex()
        # self.location_ids[index] = self.lineEdit_fovName.text()  # No longer needed
        # print(self.location_ids[index])
        location_str = 'x: ' + str(round(self.location_list[index,0],3)) + \
            ' mm, y: ' + str(round(self.location_list[index,1],3)) + \
                ' mm, z: ' + str(1000*round(self.location_list[index,2],3)) + ' um' + \
                    ', ID: ' + self.location_ids[index]
    ...
    def remove_location(self):
        ...
        if index >=0:
            ...
            print('Removing x', x, 'y', y, 'z', z)
            print('Removing', self.location_ids[index])
            print('Removing location list', self.location_list[index])
    ...
    def next(self):
        ...
        # self.go_to(index)
        # # first escape z
        # self.stage.move_z(-2) # drop 2 mm
        # # self.stage.wait_till_operation_is_completed()
        ...
        # wait for XY to finish moving before Z
        # self.stage.wait_till_operation_is_completed()
    ...
    def offset_all_XY(self):
        # current XYZ point
        xCurr = self.stage.get_pos().x_mm
        yCurr = self.stage.get_pos().y_mm
        # zCurr = self.stage.get_pos().z_mm

        # current selected coord
        index = self.dropdown_location_list.currentIndex()
        xSelect = self.location_list[index,0]
        ySelect = self.location_list[index,1]
        # zSelect = self.location_list[index,2]

        # compute offset between current point and the selected point
        xOffset = xCurr - xSelect
        yOffset = yCurr - ySelect
        # zOffset = zCurr - zSelect
        self.location_list[:,0] += xOffset # update all points
        self.location_list[:,1] += yOffset
        # self.location_list[:,2] += zOffset
        # update the GUI
        self.update_GUI_location_list()

    def offset_all_Z(self):
        # current XYZ point
        # xCurr = self.stage.get_pos().x_mm
        # yCurr = self.stage.get_pos().y_mm
        zCurr = self.stage.get_pos().z_mm
        ...
        # yOffset = yCurr - ySelect
        zOffset = zCurr - zSelect
        # self.location_list[index,0] += xOffset # update current point
        # self.location_list[index,1] += yOffset
        self.location_list[:,2] += zOffset
        # update the GUI
        self.update_GUI_location_list()

    def offset_current_XY(self):
        # current XYZ point
        xCurr = self.stage.get_pos().x_mm
        yCurr = self.stage.get_pos().y_mm
        # zCurr = self.stage.get_pos().z_mm
    ...
    def offset_current_Z(self):
        ...
        # self.location_list[index,0] += xOffset # update current point
        # self.location_list[index,1] += yOffset
        self.location_list[index,2] += zOffset
        # update the GUI
        self.update_GUI_location_list()

    def update_GUI_location_list(self):
        allFovNames = self.location_ids.copy()
        
        # Set the correct number of rows in the table
        self.table_location_list.setRowCount(self.location_list.shape[0])
        
        # update GUI location table list
        for i in range(self.location_list.shape[0]):
            self.table_location_list.setItem(i,0, QTableWidgetItem(str(round(self.location_list[i,0],3)))) # X
            self.table_location_list.setItem(i,1, QTableWidgetItem(str(round(self.location_list[i,1],3)))) # Y
            self.table_location_list.setItem(i,2, QTableWidgetItem(str(round(1000*self.location_list[i,2],1)))) # Z
            self.table_location_list.setItem(i, 3, QTableWidgetItem(self.location_ids[i])) # ID

        # current ID goes back to previous one for some reason
        self.location_ids = allFovNames
        
        # Update the dropdown location list
        self.dropdown_location_list.clear()
        for i in range(self.location_list.shape[0]):
            location_str = 'x: ' + str(round(self.location_list[i,0],3)) + \
                ' mm, y: ' + str(round(self.location_list[i,1],3)) + \
                    ' mm, z: ' + str(1000*round(self.location_list[i,2],3)) + ' um' + \
                        ', ID: ' + self.location_ids[i]
            self.dropdown_location_list.addItem(location_str)
        ...
        #     self.table_location_list.setItem(i,2, QTableWidgetItem(str(round(1000*self.location_list[i,2],1)))) # Z
        #     self.table_location_list.setItem(i, 3, QTableWidgetItem(self.location_ids[i])) # ID
        #     print('Set index', i, 'to', self.location_ids[i])
        index = self.dropdown_location_list.currentIndex()
    ...
    def cell_was_changed(self,row,column):
        ...
        location_str = 'x: ' + str(round(self.location_list[row,0],3)) + \
            ' mm, y: ' + str(round(self.location_list[row,1],3)) + \
                ' mm, z: ' + str(1000*round(self.location_list[row,2],3)) + ' um' + \
                    ', ID: ' + self.location_ids[row]
        self.dropdown_location_list.setItemText(row, location_str)
        # Remove lineEdit_fovName.setText - no longer exists
        # self.lineEdit_fovName.setText(self.location_ids[row])
        # self.go_to(row) do NOT move to point automatically
    ...
    def export_location_list(self):
        """Export location lists as CSV files. If samples are defined, save to each sample's base path. Otherwise, export current location list."""
        import pandas as pd
        import os
        from pathlib import Path
        
        # check for duplicate FOV IDs excluding nans
        duplicates = self.find_duplicates(self.location_ids)
        try:
            duplicates.remove('') # exclude blanks
            duplicates.remove('nan') # exclude nans
        except:
            pass
        if len(duplicates) > 0:
            msg = QMessageBox()
            msg.setText(f"Duplicate FOV IDs found: {duplicates}. Please fix before exporting.")
            msg.exec_()
            # return 
        
        if self.sample_list:
            # Export coordinates for each sample to their respective base paths
            saved_files = []
            failed_samples = []
            
            for i, sample in enumerate(self.sample_list):
                try:
                    # Get sample's base path
                    base_path = sample.get('base_path', '')
                    if not base_path or not os.path.exists(base_path):
                        failed_samples.append(f"Sample {i+1}: Invalid base path ({base_path})")
                        continue
                    
                    # Get coordinates for this sample
                    location_list = sample.get('location_list', [])
                    location_ids = sample.get('location_ids', [])
                    
                    if len(location_list) == 0:
                        failed_samples.append(f"Sample {i+1}: No coordinates to save")
                        continue
                    
                    # Create CSV filename
                    sample_name = sample.get('name', f'Sample_{i+1}').replace(' ', '_')
                    csv_filename = f"coords_{sample_name}.csv"
                    csv_path = os.path.join(base_path, csv_filename)
                    
                    # Create DataFrame
                    location_list_df = pd.DataFrame(location_list, columns=['x (mm)', 'y (mm)', 'z (mm)'])
                    location_list_df['z (um)'] = location_list_df['z (mm)'] * 1000  # Convert z to micrometers
                    location_list_df['ID'] = location_ids[:len(location_list)] if len(location_ids) > 0 else [f"FOV_{j}" for j in range(len(location_list))]
                    location_list_df['i'] = 0
                    location_list_df['j'] = 0
                    location_list_df['k'] = 0
                    
                    # Reorder columns to match expected format
                    location_list_df = location_list_df[['x (mm)', 'y (mm)', 'z (um)', 'ID', 'i', 'j', 'k']]
                    location_list_df.to_csv(csv_path, index=False, header=True)
                    
                    saved_files.append(f"{sample_name}: {csv_path}")
                    print(f"Coordinates saved for {sample_name}: {csv_path}")
                    
                except Exception as e:
                    failed_samples.append(f"Sample {i+1}: {str(e)}")
            
            # Show results to user
            msg = QMessageBox()
            msg.setWindowTitle("Export Results")
            
            if saved_files and not failed_samples:
                msg.setText(f"Successfully saved coordinates for {len(saved_files)} samples to their respective base paths.")
                msg.setDetailedText("\n".join(saved_files))
                msg.setIcon(QMessageBox.Information)
            elif saved_files and failed_samples:
                msg.setText(f"Saved {len(saved_files)} files successfully, but {len(failed_samples)} failed.")
                msg.setDetailedText("Successful:\n" + "\n".join(saved_files) + "\n\nFailed:\n" + "\n".join(failed_samples))
                msg.setIcon(QMessageBox.Warning)
            else:
                msg.setText("Failed to save coordinates for any samples.")
                msg.setDetailedText("\n".join(failed_samples))
            ...
            # Export current location list (original behavior) - user chooses location
            ...
            if file_path:
                ...
                location_list_df['z (um)'] = location_list_df['z (mm)'] * 1000  # Convert z to micrometers for compatibility
                ...
                msg = QMessageBox()
                msg.setText(f"Coordinates exported successfully to:\n{file_path}")
                msg.exec_()

    def import_location_list(self):
        file_path, _ = QFileDialog.getOpenFileName(self, "Import Location List", '', "CSV Files (*.csv);;All Files (*)")
        if file_path:
            self.lineEdit_savingDir.setText(str(Path(file_path).parent)) # rename saving direction to path
...
class NapariLiveWidget(QWidget):
    ...
    signal_newAnalogGain = Signal(float)
    signal_autoLevelSetting = Signal(bool)
    signal_selected_channels = Signal(list)
    ...
    def update_config_exposure_time(self, new_value):
        ...
        self.signal_newExposureTime.emit(new_value)
        # Emit selected channels based on exposure times
        self.emit_selected_channels()

    def get_selected_channels(self):
        """Return list of configuration names where exposure time > 0"""
        selected_channels = []
        for config in self.configurationManager.configurations:
            if config.exposure_time > 0:
                selected_channels.append(config.name)
        return selected_channels
    ...
    def on_configuration_changed(self, config_name, parameter, value):
        """Handle configuration changes and emit selected channels if exposure time changed."""
...
class NapariMultiChannelWidget(QWidget):
    ...
    def initChannels(self, channels):
        ...
        if channels and isinstance(channels[0], dict):
            # Extract channel names from dictionaries
            self.channels = set(channel['name'] for channel in channels)
        else:
